...
import type { LegRoutes, ResolvedPlace } from "./tripAddUtils.ts"
import { decodePolyline } from "./tripAddUtils.ts"
...
export function AirportPinsMap(props: {
    ...
    const uniqueAirports = props.airports.filter((airport, idx, arr) => {
        return arr.findIndex((item) => item.airport.code === airport.airport.code) === idx
    })

    const points = uniqueAirports.map((airport) => ({
        lat: airport.airport.lat,
        lng: airport.airport.lng
    }))
    ...
    return (
        <Box sx={{ height: 280, borderRadius: 3, overflow: "hidden", border: "1px solid rgba(47,65,86,0.12)" }}>
            <Map
                ...
                fullscreenControl={false}
            >
                {uniqueAirports.map((airport, idx) => {
                    const isFirst = idx === 0

                    return (
                        <AdvancedMarker
                            key={`${airport.airport.code}-${idx}`}
                            position={{ lat: airport.airport.lat, lng: airport.airport.lng }}
                        >
                            <Pin
                                background={isFirst ? "#2E7D32" : "#1565C0"}
                                glyphColor="#fff"
                                borderColor={isFirst ? "#1B5E20" : "#0D47A1"}
                            />
                        </AdvancedMarker>
                    )
                })}
