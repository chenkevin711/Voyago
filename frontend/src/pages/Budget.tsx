...
export default function Budget() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const t = await getPlannedTripById(tripId); // ✅ await async helper
  ...
  const breakdown = useMemo(() => {
    ...
    return [
      ...
      {
        label: "Stay",
        amount: trip.selectedAccommodation ? trip.selectedAccommodation.nightlyRate * nights : 0,
      },
      {
        label: "Attractions",
        amount: (trip.selectedAttractions ?? []).reduce((sum, item) => sum + (item.price ?? 0), 0),
      },
      {
        label: "Ground transport",
        amount: (trip.navigationPlans ?? []).reduce((sum, plan) => sum + (plan.estimatedCost ?? 0), 0),
      },
    ];
  }, [trip]);
